...
namespace EffectClasses
{
	...
	public static class BitmapEffects
	{
		...
		/// <returns></returns>
		public static Bitmap Render(Bitmap image, double percent, params Point[] points)
		{
			Bitmap bm = new Bitmap(image);
			
			//Get BitmapData of the whole image by locking the bits.
			BitmapData data = bm.LockBits(new Rectangle(new Point(0, 0), bm.Size),
				ImageLockMode.ReadWrite, image.PixelFormat);

			//and a byte array having room for all the bytes
			byte[] bytes = new byte[bm.Height * data.Stride]; 
		...
		private static void recPixelRender(BitmapData data, ref byte[] bytes, 
